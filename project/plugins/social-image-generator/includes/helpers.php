...
function get_template($name, $args = []) {
    set_query_var(SOCIAL_IMAGE_GENERATOR_SLUG, $args);

    $path = SOCIAL_IMAGE_GENERATOR_TEMPLATES_PATH . $name . '.php';

    if (!file_exists($path)) {
        return sprintf(
            /* translators: %s is the name of the template that's being included */
            __('Error: No template file found for the %s part.', 'social-image-generator'),
...
function get_template_var($name, $default = null) {
    $query_var = get_query_var(SOCIAL_IMAGE_GENERATOR_SLUG);
